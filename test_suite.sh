...
# Test parameters
IOR_NODES=4              # Adjust to your cluster
...
OUTPUT=$(mpirun -n 1 ior -w -o ${TESTDIR}/ior_single -t $BLOCKSIZE -b $FILESIZE -F 2>&1)
...
OUTPUT=$(mpirun -n 1 ior -r -o ${TESTDIR}/ior_single -t $BLOCKSIZE -b $FILESIZE -F 2>&1)
...
OUTPUT=$(mpirun -n $TOTAL_TASKS ior -w -o ${TESTDIR}/ior_multi -t $BLOCKSIZE -b $FILESIZE_MULTI -F 2>&1)
WRITE_BW=$(echo "$OUTPUT" | grep "write" | awk '{print $3}')
...
OUTPUT=$(mpirun -n $TOTAL_TASKS ior -r -o ${TESTDIR}/ior_multi -t $BLOCKSIZE -b $FILESIZE_MULTI -F 2>&1)
READ_BW=$(echo "$OUTPUT" | grep "read" | awk '{print $3}')
...
OUTPUT=$(mpirun -n $TOTAL_TASKS ior -w -o ${TESTDIR}/ior_shared -t $BLOCKSIZE -b 1g 2>&1)
WRITE_BW=$(echo "$OUTPUT" | grep "write" | awk '{print $3}')
...
OUTPUT=$(mpirun -n $((TOTAL_TASKS/2)) ior -w -r -o ${TESTDIR}/ior_small -t 64k -b 1g -F 2>&1)
...
OUTPUT=$(mpirun -n $MDTEST_TASKS mdtest -n 10000 -d ${TESTDIR}/mdtest -F 2>&1)
...
OUTPUT=$(mpirun -n 2 --map-by node osu_bw 2>&1 | tail -1)
...
OUTPUT=$(mpirun -n 2 --map-by node osu_latency 2>&1 | grep "^8 " || echo "8 0")
